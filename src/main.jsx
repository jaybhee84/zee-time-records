...
function getAppIconPath() {
  if (process.platform === "win32") {
    return path.join(__dirname, "../src/assets/ZeeTimeRecords.ico");
  } else if (process.platform === "darwin") {
    return path.join(__dirname, "../src/assets/ZeeTimeRecords.icns");
  }
  return path.join(__dirname, "../src/assets/ZeeTimeRecords.png");
}
...
// Cross-platform USB Drive Detection (Windows & macOS)
ipcMain.handle("get-usb-drives", async () => {
  if (process.platform === "win32") {
    try {
      ...
      console.error("Failed to enumerate Windows removable drives:", err);
      return [];
    }
  } else if (process.platform === "darwin") {
    try {
      ...
      for (const vol of volumes) {
        ...
        try {
          ...
          if (vol === "Macintosh HD" || vol === "System") continue;

          drives.push({
            label: `${vol} (/Volumes/${vol})`,
            path: fullPath,
          });
        } catch (e) {
      ...
      console.error("Failed to enumerate macOS removable drives:", err);
      return [];
    }
  }
...
ipcMain.handle("auth-login", async (event, { username, password } = {}) => {
  ...
  const user = getOne("SELECT * FROM users WHERE username = ?", [
    (username || "").trim(),
  ]);
...
ipcMain.handle(
  ...
  async (event, { username, password } = {}) => {
    ...
    const existing = getOne("SELECT * FROM users WHERE username = ?", [
      username.trim(),
    ]);
    if (existing) {
...
ipcMain.handle(
  "auth-update-user",
  async (event, { id, username, password } = {}) => {
    ...
    if (password) {
      ...
      runQuery("UPDATE users SET username = ? WHERE id = ?", [
        username.trim(),
        id,
      ]);
    }
  ...
  try {
    ...
    if (!reader.getTableNames().includes("Employees")) {
      return {
        success: false,
        error:
          'No "Employees" table found in this file — is this a Vinea backup?',
      };
    }
...
ipcMain.handle(
  "add-custom-subgroup",
  async (event, { groupName, subGroupName } = {}) => {
    try {
      ...
      if (!trimmedGroup || !trimmedSubGroup) {
        return {
          success: false,
          error: "Group and sub-group name are required.",
        };
      }
      ...
      return {
        success: true,
        subgroups: getAll(
          "SELECT * FROM custom_subgroups ORDER BY subGroupName ASC",
        ),
      };
    } catch (err) {
...
ipcMain.handle(
  ...
  async (event, { category, amIn, amOut, pmIn, pmOut, graceMinutes } = {}) => {
    try {
      ...
      if (
        ![amIn, amOut, pmIn, pmOut].every((t) =>
          timePattern.test(String(t || "")),
        )
      ) {
        return {
          success: false,
          error: "All four time fields are required (HH:MM).",
        };
      }
...
ipcMain.handle("load-employees", async () => {
  try {
    return getAll(
      "SELECT * FROM employees ORDER BY familyName ASC, firstName ASC",
    );
  } catch (err) {
...
ipcMain.handle(
  "save-punches",
  async (event, { pin, year, month, newPunches = [] } = {}) => {
    try {
      const monthFormatted = String(month).padStart(2, "0");
      const pattern = `${year}-${monthFormatted}-%`;
...
ipcMain.handle("open-dtr-pdf-external", async (event, payload = {}) => {
  ...
  try {
    ...
    const safeName = (suggestedName || "DTR.pdf").replace(
      /[/\\?%*:|"<>]/g,
      "-",
    );
...
// Sends a PDF file to a printer cross-platform without Chromium margin issues
function printPdfFile(pdfPath, deviceName) {
  return new Promise((resolve, reject) => {
    const { execFile } = require("child_process");

    if (process.platform === "win32") {
      // Windows: SumatraPDF silent background printer
    ...
    // macOS / Linux: CUPS printing via standard `lp` terminal command
